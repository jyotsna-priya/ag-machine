        ...
        $conn = mysqli_connect("ec2-54-161-132-160.compute-1.amazonaws.com", "server", "password123", "agmachinedb");
            ...
            if ($conn->connect_error) {
             ...
             if(!empty($email) && !empty($password)){
                ...
                if($role == 'F') {
                    $sql="SELECT password FROM farmers WHERE email_id = '$email'";
                    echo $sql;
                    $output= $conn->query($sql);
                    $row = $output->fetch_assoc();
                    if($output->num_rows > 0 && ($row['password'] == $password)){
                       //Get farmer id
  			$sql = "SELECT farmer_id FROM farmers WHERE email_id = '$email'";
  			...
  			if (mysqli_num_rows($result) == 1) {
		       ...
		       echo "<script>window.location='./farmersdashboard.php';</script>";
                       session_start();
